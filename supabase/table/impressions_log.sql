...
-- CTR計算用（work_id + 時間範囲）
create index if not exists idx_impressions_work_time 
  on public.impressions_log (work_id, impressed_at);

-- ユーザー別分析用（user_id が null でない場合）
create index if not exists idx_impressions_user_time 
  on public.impressions_log (user_id, impressed_at) 
  where user_id is not null;

-- 推薦効果測定用
create index if not exists idx_impressions_type_context 
  on public.impressions_log (impression_type, page_context);

-- セッション分析用
create index if not exists idx_impressions_session 
  on public.impressions_log (session_id, impressed_at);

-- セッション内重複チェック用
create unique index if not exists idx_impressions_session_work_unique 
  on public.impressions_log (session_id, work_id) 
